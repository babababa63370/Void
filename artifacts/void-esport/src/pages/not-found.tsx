...
export default function NotFound() {
  ...
  const navLinks = [
    { href: "/about", label: t("nf_about"), icon: <Sword className="w-4 h-4" /> },
    { href: "/roster", label: t("nf_roster"), icon: <Users className="w-4 h-4" /> },
    { href: "/achievements", label: t("nf_achievements"), icon: <Trophy className="w-4 h-4" /> },
    { href: "/join", label: t("nf_join"), icon: <Home className="w-4 h-4" /> },
  ];
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-3xl mx-auto w-full">
        ...
        >
          <Link href="/" className="flex items-center gap-3 group">
            <img src={logoPath} alt="VOID Logo" className="w-12 h-12 object-contain opacity-80 group-hover:opacity-100 transition-opacity" />
        ...
        >
          <span
            ...
            style={{ letterSpacing: "-0.04em", textShadow: "0 0 40px hsl(270 91% 65% / 0.6), 0 0 80px hsl(270 91% 65% / 0.3)" }}
          >
            404
          </span>
          <motion.span
            className="absolute inset-0 font-orbitron font-black text-[clamp(6rem,20vw,13rem)] leading-none text-cyan-400 opacity-0"
            style={{ letterSpacing: "-0.04em", clipPath: "inset(25% 0 55% 0)" }}
          ...
          <motion.span
            ...
            style={{ letterSpacing: "-0.04em", clipPath: "inset(58% 0 18% 0)" }}
            animate={{ opacity: [0, 0.6, 0], x: [0, 6, 0] }}
            transition={{ duration: 0.1, repeat: Infinity, repeatDelay: 3.5, delay: 0.06 }}
            aria-hidden
          >
            404
          </motion.span>
        ...
        >
          {socialLinks.map((s) => (
            <a key={s.href} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="text-muted-foreground hover:text-primary transition-colors">
              {s.icon}
            </a>
          ))}
        </motion.div>
        ...
        >
          © {new Date().getFullYear()} VOID Esport — {t("nf_copyright")}
        </motion.p>
      </div>
    </div>
  );
